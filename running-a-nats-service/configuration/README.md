...
| `proxy_protocol`    |  Enable proxy support for client connections. Auto-detects the standard HA_proxy headers and removes it before the TLS handshake. When not enabled standard-compliant proxies sending a proxy header will result in connection failure.  |    `false`  |
| `allow_non_tls`                        | Allow non_tls nats client and MQTT connection even if tls is configure. This is an advanced option. Use with care as it may undermine the intended security.                                                        |    `false` by default TLS is required if the tls section is configured.    |
...
| `write_timeout` |  `default`,`retry`,`close` - If close is set the connection will be aggressively closed on first write timeout without waiting for `write_deadline`  | `default` (currently `retry`) |
...
| `no_system_account`                     |  Disable the system account. Occasionally used for trivial embedded servers | `false` System account is created |  |
| `no_header_support`                     |  Disables message header support. For enforcing backwards compatibility with NATS v1.x  | `false` messages headers supported |  |
...
| `max_closed_clients`                                                                                            | The server remembers closed client connection for monitoring purposes. The default maybe to high for memory constrained environments or too low for connection error tracking.        | `10000`    |         |
...
| `trace_headers`                                                                                    | If `true` only message headers are traced, rather than the full message.                                                                                                                                               | `false`, traces body         |         |
...
| `logtime_utc`                                                                                          | If set to `true`, log in UTC independent of local server time                               | `false`, use local time |         |


| `log_file`                                                                                         | Log file name, relative to...                                                                                                                                                                                            | No log file               |         |
| [`log_size_limit`/`logfile_size_limit`](/running-a-nats-service/configuration/logging.md#using-the-configuration-file)  | Size in bytes after the log file rolls over to a new one                                                                                                                                                                 | `0`, unlimited            |         |
...
| `ports_file_dir`                                                                                    | Directory to write a file containing the servers' open ports to, relative to ...                                                                                                                                         | (not set)                 |         |
